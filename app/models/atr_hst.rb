class AtrHst
  ...
  scope :by_attr_and_value_different, lambda{|attr_id, different_to| where(:atr_id => Atr.find_by(id: attr_id) , :value.ne => different_to.to_s )}
  ...
  scope :by_attr_and_value_different_ts, lambda{|attr_id, different_to , ts| where(:atr_id => Atr.find_by(id: attr_id), :value.ne => different_to.to_s , :tstamp.in => ts )}
